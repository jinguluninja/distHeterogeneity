...
from nets_clasification import *
...
class DistrSystem(object):
    ...
    def get_num_iters_per_cycle(self, train_sets):
        self.inst_iters_per_cycle = [0 for i in range(len(self.FLAGS.num_inst))]
    ...
    def get_lr(self, train_sets):
        self.inst_lr = [self.FLAGS.lr for i in range(len(self.FLAGS.num_inst))]
    ...
    def get_train_sample(self, train_set):
        ...
        weights = [self.sample_weights_dict[x] for x in train_sets]
    ...
    def train(self, session, train_sets, val_set):
        ...
        for cycle in range(max_cycles):
            for inst in range(self.FLAGS.num_inst):
                inst_iters = self.inst_iters_per_epoch[inst]
                for i in range(inst_iters):
                    ...
                    logging.info('Iter: %s, Cycle: %s, Inst: %x, train loss: %.3f, train acc: %.3f' % (tot_iters, cycle, inst, train_loss, train_acc))
                    tot_iters += 1           
